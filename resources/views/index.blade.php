...
<table>
	...
	@foreach($data as $keglaraveldata)
		<tr>
			...
			<td><span style="cursor:pointer;"><a onClick="deleteKeg({{ $keglaraveldata->kegId }})">Delete</a></span></td>
			<td><span style="cursor:pointer;"><a href="{{ route('keg.edit', $keglaraveldata->kegId) }}">Update</span></a></td>
	@endforeach
</tr>
</table>
</div>
...
function deleteKeg(kegId)
{
	$.ajax({url:'keg/'+kegId, type: 'DELETE', success: function(data){
	$.get('keg', function(data){
		alert('Entry with Keg Id'+' '+kegId+' '+'has been deleted!');
		location.reload();
	})
}
});
